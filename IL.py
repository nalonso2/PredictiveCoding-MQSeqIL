...
class IL(nn.Module):
    def __init__(self, layer_szs, n_iter=25, gamma=.015, type=0, bias=True, smax=True, linear=False,
                 func=nn.ReLU(), func_d=relu_d, alpha=1, beta=.5, lr_min=.001, r=.000001, v=.9999):
        super().__init__()
        ...
        self.lr_min = lr_min
        self.wt_var = [alpha for x in range(len(self.wts))]
        self.r = r
    ...
    def compute_targets(self, h, global_target):
        with torch.no_grad():
            ...
            if self.smax:
                ...
                targ[-1] = (1 - self.mod_prob) * global_target.clone() + self.mod_prob * torch.sigmoid(targ[-1])
            ...
            for t in range(self.T):
                ...
                for layer in reversed(range(1, self.num_layers - 1)):
                    ...
                    if layer < self.num_layers - 2:
                        ...
                        if self.smax:
                            ...
                            err = (targ[-1] - torch.sigmoid(p))  # Binary cross-ent with sigmoid

                    # Update Hidden Activities
                    dfdt = err.matmul(self.wts[layer][1].weight) * self.func_d(targ[layer])
                ...
                if self.smax:
                    ...
                    targ[-1] = (1 - self.mod_prob) * global_target + self.mod_prob * torch.sigmoid(self.wts[-1](targ[-2]))
    ...
    def LMS_update(self, targ):
        ...
        for i in range(self.num_layers - 1):
            ...
            with torch.no_grad():
                ...
                self.wts[i][-1].weight.grad = e.t().matmul(self.func(targ[i])) / e.size(0)
                self.wts[i][-1].bias.grad = torch.mean(e, dim=0)

                # Update weights with normalized step size and precision weighting
                self.wts[i][-1].weight.data += self.wts[i][-1].weight.grad * self.alpha
                self.wts[i][-1].bias.data += self.wts[i][-1].bias.grad * self.alpha


    def MQ_update(self, targ):
        # Update each weight matrix
        for i in range(self.num_layers - 1):
            # Compute local losses, sum neuron-wise and avg batch-wise
            with torch.no_grad():
    ...
    def ImSGD_Update(self, x, y, gl_target, beta=1):
        ...
        for iter in range(500):
            ...
            with torch.no_grad():
                prox = loss
                for i in range(self.num_layers - 1):
                    prox += .5 / beta * torch.square(self.wts[i][-1].weight - wts_b[i]).sum()
                    prox += .5 / beta * torch.square(self.wts[i][-1].bias - bias_b[i]).sum()
