...
class IL(nn.Module):
    def __init__(self, lr=.001, type=0, n_iter=25, gamma=.05, alpha=0, lr_min=.001, r=.000001, decay=False, beta=100, arch=0):
        super().__init__()
        ...
        self.gamma = gamma
        self.lr_min = lr_min
        self.wt_var = [alpha for x in range(5)]
        self.r = r
        self.decay = decay
        ...
        if arch == 0:
            self.wts = nn.Sequential(
                nn.Sequential(
                    nn.Conv2d(3, 32, kernel_size=(5, 5), stride=(2, 2), bias=True),
                    nn.ReLU()
                ),


                nn.Sequential(
                    nn.Conv2d(32, 64, kernel_size=(5,5), stride=(2,2), bias=True),
                    nn.ReLU()
                ),
            ...
            self.wts = nn.Sequential(
                nn.Sequential(
                    nn.Conv2d(3, 32, kernel_size=(3, 3), stride=(1, 1), bias=True),
                    nn.ReLU(),
                    nn.MaxPool2d(2, 2),
                ),

                nn.Sequential(
                    nn.Conv2d(32, 64, kernel_size=(3, 3), stride=(1, 1), bias=True),
                    nn.ReLU(),
                    nn.MaxPool2d(2, 2),
                ),
                ...
                nn.Sequential(
                    nn.Conv2d(128, 256, kernel_size=(3, 3), stride=(1, 1), bias=True),
                    nn.ReLU(),
                    nn.MaxPool2d(2, 2),
                    nn.Flatten()
                ),
